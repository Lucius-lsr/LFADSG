...
flags.DEFINE_integer("C", 100, "Number of conditions")  # different rates (neurons?)
flags.DEFINE_integer("N", 50, "Number of units for the RNN")  # recorded time
flags.DEFINE_integer("S", 50, "Number of sampled units from RNN")  # recorded time
flags.DEFINE_integer("npcs", 10, "Number of PCS for multi-session case.")
flags.DEFINE_float("train_percentage", 4.0 / 5.0,
                   "Percentage of train vs validation trials")
flags.DEFINE_integer("nreplications", 40,
                     "Number of noise replications of the same underlying rates.")  # same rate but randomly sampled
...
for n in range(ndatasets):
    ...
    # x0 is random seed corresponding to a single condition (100) and a single random repetition (40), 100*40
    rates, x0s, inputs = generate_data(rnn, T=T, E=E, x0s=x0s, P_sxn=P_sxn, input_magnitude=input_magnitude,
                                       input_times=input_times)

    # from rates to binary data with random noise
    ...
    # print(rates[100].shape)
    # print(noisy_data[100].shape)
    # print(inputs[10][10])
    # print(condition_labels[0])
    # print(input_times[0].shape)
    # exit()
    ...
    data = {'train_truth': rates_train,   # label
            'valid_truth': rates_valid,
            'input_train_truth': input_train,  # additional data
            'input_valid_truth': inputs_valid,
            'train_data': noisy_data_train,  # data
            'valid_data': noisy_data_valid,
            'train_percentage': train_percentage,
            'nreplications': nreplications,
            'dt': rnn['dt'],
            'input_magnitude': input_magnitude,
            'input_times_train': input_times_train,  # times of hit
