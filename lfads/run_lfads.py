...
from lfads_gragh import LFADSG
import numpy as np
import os
...
NUM_NODE = 50  # define number of nodes
...
BATCH_SIZE = 128  # 128
LEARNING_RATE_INIT = 0.000000001  # 0.01 originally
LEARNING_RATE_DECAY_FACTOR = 0.95
LEARNING_RATE_STOP = 0.000000000001  # 0.00001
...
# MODIFIED HYPER
flags.DEFINE_integer("dim_e", DIM_E, "dimension of E_t")
flags.DEFINE_integer("dim_c", DIM_C, "dimension of C_t")
flags.DEFINE_list("dim_gcn_hidden", DIM_GCN_HIDDEN, "dimension of hidden layers in gcn")


# DATA
flags.DEFINE_integer("num_node", NUM_NODE, "Data number of node")
...
def build_model(hps, kind="train", datasets=None):
    ...
    with tf.variable_scope("LFADS", reuse=None):
        model = LFADSG(hps, kind=build_kind, datasets=datasets)  # modification here
...
def build_hyperparameter_dict(flags):
    ...
    d['dim_e'] = flags.dim_e
    d['dim_c'] = flags.dim_c
    d['dim_gcn_hidden'] = flags.dim_gcn_hidden
