...
"""
@Time    : 7/30/20 1:39 PM
@Author  : Lucius
@FileName: lfads_gragh.py
@Software: PyCharm
"""
...
class LFADSG(object):
    ...
    def __init__(self, hps, kind="train", datasets=None):
        ...
        # key variables
        num_node = hps.num_node
        num_steps = hps.num_steps
        dim_e = hps.dim_e
        dim_c = hps.dim_c
        dim_gcn_hidden = hps.dim_gcn_hidden

        ext_input_dim = hps.ext_input_dim
        cell_class = EncoderGRU
        gen_cell_class = GraphGRU
        ...
        if hps.output_dist == 'poisson':
            ...
            assert np.issubdtype(datasets[hps.dataset_names[0]]['train_data'].dtype,
                                 int), "Data dtype must be int for poisson output distribution"
            data_dtype = tf.int32
        elif hps.output_dist == 'gaussian':
            assert np.issubdtype(datasets[hps.dataset_names[0]]['train_data'].dtype,
                                 float), "Data dtype must be float for gaussian output dsitribution"
        ...
        def encode_data(dataset_bxtxd, enc_cell, name, forward_or_reverse,
            ...
            with tf.variable_scope(name + "_enc" + dstr, reuse=False):
                enc_state = tf.tile(
                    tf.Variable(tf.zeros([1, num_node, enc_cell.state_size]),
                                name=name + "_enc_h0" + dstr), [batch_size, 1, 1])
                enc_state.set_shape([None, num_node, enc_cell.state_size])  # tile loses shape
            ...
            for i, t in enumerate(time_fwd_or_rev):
                with tf.variable_scope(name + "_enc" + dstr, reuse=True if i > 0 else None):
                    ...
                    enc_input_t_bxfpe = tf.expand_dims(enc_input_t_bxfpe, -1)
        ...
        # generate initial graph, priors and posteriors
        with tf.variable_scope("z", reuse=False):

            self.prior_graph_0 = None
            self.posterior_graph_0 = None
            ic_enc = tf.concat(axis=2, values=[ic_enc_fwd[-1], ic_enc_rev[0]])
            ic_enc = tf.nn.dropout(ic_enc, keep_prob)
            node_num = ic_enc.get_shape().as_list()[1]
            self.prior_graph_0 = LearnableGraph(batch_size, node_num, name="prior_graph0")
            self.posterior_graph_0 = GraphFromInput(ic_enc)

            # Priors for controller, 'co' for controller output
            self.prior_zs_co = prior_zs_co = [None] * num_steps
            self.posterior_graph_t = posterior_graph_t = [None] * num_steps
        ...
        # output of encoder -> sigma -> probability ->(graph sample) graph
        #                   ->(graph) mean/var ->(sample) C ->(gcn) output
        self.graph_t = graph_t = [None] * num_steps
        self.graph_states = graph_states = [None] * num_steps
        self.graph_outs = graph_outs = [None] * num_steps
        self.graph_rnn_inputs = graph_rnn_inputs = [None] * num_steps

        con_cell = ControllerGRU(weight_scale=hps.cell_weight_scale, clip_value=hps.cell_clip_value)

        graph_states[-1] = self.posterior_graph_0.prob

        gen_cell = gen_cell_class(dim_c,
                                  weight_scale=hps.gen_cell_input_weight_scale,
                                  clip_value=hps.cell_clip_value)
        with tf.variable_scope("graph_rnn", reuse=False):
            graph_rnn_state = tf.tile(tf.Variable(tf.zeros([1, num_node, dim_c]), name="c0"), [batch_size, 1, 1])
            graph_rnn_state.set_shape([None, num_node, dim_c])  # tile loses shape

            self.graph_rnn_states = graph_rnn_states = [None] * num_steps
            self.graph_rnn_outs = graph_rnn_outs = [None] * num_steps
            self.posterior_c_t = posterior_c_t = [None] * num_steps
            self.c_t = c_t = [None]*num_steps
            graph_rnn_states[-1] = graph_rnn_state
            graph_rnn_outs[-1] = gen_cell.output_from_state(graph_rnn_states[-1])
        ...
        for t in range(num_steps):
            ...
            graph_rnn_in_t = tf.concat(axis=2, values=con_in_list_t)
            graph_rnn_in_t = tf.nn.dropout(graph_rnn_in_t, keep_prob)

            # from output of encoder to get \Sigma_t
            sigma_t = GraphFromInput(graph_rnn_in_t).prob

            # from sigma to get graph
            with tf.variable_scope("con", reuse=True if t > 0 else None):
                graph_outs[t], graph_states[t] = con_cell(sigma_t, graph_states[t - 1])  # RNN unit
                posterior_graph_t[t] = GraphFromProb(graph_outs[t])
            if kind == "train":
                graph_t[t] = posterior_graph_t[t].sample
            elif kind == "posterior_sample_and_average":
                graph_t[t] = posterior_graph_t[t].sample
            elif kind == "posterior_push_mean":
                graph_t[t] = posterior_graph_t[t].mean
            else:
                pass
                # graph_t[t] = prior_zs_ar_con.samples_t[t]

            # Inputs to the graphRNN (controller output + external input)
            graph_rnn_inputs[t] = graph_rnn_in_t

            # GraphRNN
            data_t_bxd = dataset_ph[:, t, :]
            with tf.variable_scope("graph_rnn", reuse=True if t > 0 else None):
                graph_rnn_outs[t], graph_rnn_states[t] = gen_cell(graph_rnn_inputs[t], graph_t[t],
                                                                  graph_rnn_states[t - 1])
                graph_rnn_outs[t] = tf.nn.dropout(graph_rnn_outs[t], keep_prob)

            # linear map to mean/variance and sample
            with tf.variable_scope("gen", reuse=True if t > 0 else None):
                posterior_c_t[t] = GraphGaussianFromInput(graph_rnn_outs[t], name='_posterior_ct')
                c_t[t] = posterior_c_t[t].sample

            with tf.variable_scope("gcn", reuse=True if t > 0 else None):
                if hps.output_dist == 'poisson':
                    gcn_out_t = graph_convolution(graph_t[t], c_t[t], dim_gcn_hidden, 1, bias=True)  # GCN
                    gcn_out_t = tf.reduce_sum(gcn_out_t, 2)
                    log_rates_t = linear(gcn_out_t, node_num, do_bias=False, normalized=True, name="gcn_2_log_rate")
                    ...
                    self.loglikelihood_t = loglikelihood_t = Poisson(log_rates_t).logp(data_t_bxd)

                elif hps.output_dist == 'gaussian':
                    mean_n_logvars = graph_convolution(graph_t[t], c_t, dim_gcn_hidden, 1, bias=True)  # GCN
                    mean_n_logvars = tf.reduce_sum(mean_n_logvars, 2)
                    ...
                    self.loglikelihood_t = loglikelihood_t = diag_gaussian_log_likelihood(data_t_bxd, means_t_bxd, logvars_t_bxd)
                else:
                    assert False, "NIY"
        ...
        if kind in ["train", "posterior_sample_and_average", "posterior_push_mean"]:
            ...
            graph0_priors = [self.prior_graph_0]
            graph0_posts = [self.posterior_graph_0]

            from graph_construct import KLCost_BinaryBinary
            kl_cost_g0_b = KLCost_BinaryBinary(graph0_priors, graph0_posts).kl_cost_b
        ...
        self.kl_weight = kl_weight = \
            tf.minimum(kl_decay_step_f / kl_increase_steps_f, 1.0)  # 1.0

        self.timed_kl_cost = kl_weight * self.kl_cost
        self.weight_corr_cost = hps.co_mean_corr_scale  # * self.corr_cost
        self.cost = self.recon_cost + self.timed_kl_cost + self.weight_corr_cost  # + self.timed_l2_cost +
        ...
        if self.hps.do_train_io_only:
            self.train_vars = tvars = \
                tf.get_collection('IO_transformations', scope=tf.get_variable_scope().name)
        ...
        # train all variables
            ...
            self.train_vars = tvars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
        ...
        for i in range(len(tvars)):
            ...
            self.param_name.append(tvars[i].name)
    ...
    def train_epoch(self, datasets, batch_size=None, do_save_ckpt=True):
        ...
        """
        ops_to_eval = [self.cost, self.recon_cost, self.kl_cost, self.kl_weight, self.train_op]
        # ops_to_eval = [self.posterior_graph_0.prob, self.graph_outs[10], self.graph_rnn_inputs[10],
        #                self.posterior_graph_t[10].prob, self.graph_t[10], self.graph_rnn_outs[10], self.c_t[10], self.loglikelihood_t,
        #                self.rates[10], self.train_op]
        # ops_to_eval = [self.ic_enc_fwd[0], self.ic_enc_rev[0], self.posterior_graph_0.prob, self.train_op]
        ...
        return epoch_total_cost, epoch_recon_cost, epoch_kl_cost, kl_weight
    ...
    def run_epoch(self, datasets, ops_to_eval, kind="train", batch_size=None, do_collect=True, keep_prob=None):
        ...
        for name, example_idxs in all_name_example_idx_pairs:
            ...
            # print('train begin')
            # grad = tf.gradients(self.cost, self.train_vars)
            # for g in grad:
            #     print(np.any(np.isnan(session.run(g))))
    ...
    def summarize_all(self, datasets, summary_values):
        ...
        train_summ, train_step = session.run([self.merged_train,
                                              self.train_step],
                                             feed_dict={self.kl_cost_ph: tr_kl_cost,
      ...
      recon: %.2f, %.2f,     kl: %.2f, %.2f,  kl weight: %.2f, " % \
            ...
            csv_outstr = "epoch,%d, step,%d, total,%.2f,%.2f, recon,%.2f,%.2f, kl,%.2f,%.2f, klweight,%.2f\n" % (
                i, train_step, tr_total_cost, ev_total_cost, tr_recon_cost, ev_recon_cost, tr_kl_cost, ev_kl_cost,
                kl_weight)

        else:
            print("Epoch:%d, step:%d TRAIN: total: %.2f     recon: %.2f, kl: %.2f,   kl weight: %.2f" % \
                  (i, train_step, tr_total_cost, tr_recon_cost, tr_kl_cost,
                   kl_weight))
            csv_outstr = "epoch,%d, step,%d, total,%.2f, recon,%.2f, kl,%.2f, klweight,%.2f \n" % \
                         (i, train_step, tr_total_cost, tr_recon_cost,
                          tr_kl_cost, kl_weight)
    ...
    def train_model(self, datasets):
        ...
        while True:
            ...
            tr_total_cost, tr_recon_cost, tr_kl_cost, kl_weight = self.train_epoch(datasets, do_save_ckpt=do_save_ckpt)
    ...
    def eval_model_runs_batch(self, data_name, data_bxtxd, ext_input_bxtxi=None, do_eval_cost=False, do_average_batch=False):
        ...
        if self.hps.co_dim > 0:
            tf_vals.append(self.generated_graphs)
        ...
        ff += 1
        if self.hps.co_dim > 0:
            generated_graphs = [np_vals_flat[f] for f in fidxs[ff]];
            ff += 1
        ...
        if self.hps.co_dim > 0:
            generated_graphs = list_t_bxn_to_tensor_bxtxn(generated_graphs)
        ...
        if E < hps.batch_size:
            ...
            if self.hps.co_dim > 0:
                generated_graphs = generated_graphs[idx, :, :]
        ...
        if do_average_batch:
            ...
            if self.hps.co_dim > 0:
                generated_graphs = np.mean(generated_graphs, axis=0)
        ...
        if self.hps.co_dim > 0:
            model_vals['generated_graphs'] = generated_graphs
    ...
    def eval_model_runs_avg_epoch(self, data_name, data_extxd, ext_input_extxi=None):
        ...
        if hps.co_dim > 0:
            generated_graphs = np.zeros([E_to_process, T, hps.co_dim])
        ...
        for es_idx in range(E_to_process):
            ...
            if self.hps.co_dim > 0:
                generated_graphs[es_idx, :, :] = model_values['generated_graphs']
        ...
        if self.hps.co_dim > 0:
            model_runs['generated_graphs'] = generated_graphs
